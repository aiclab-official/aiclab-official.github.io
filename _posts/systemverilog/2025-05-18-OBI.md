...
tags:
  - OBI
excerpt: "In this tutorial, we'll demystify OBI by designing and verifying a minimal, compliant master and slave from the ground up."
header:
...
- [Part 3: Designing an OBI Slave](#part-3-designing-an-obi-slave)
- [Part 4: Designing an OBI Master](#part-4-designing-an-obi-master)
...
## Part 3: Designing an OBI Slave

Our first task is to create an OBI slave. This component will act like a small memory, responding to read and write requests from a master.

<figure>
  <img src="/assets/images/18- OBI_Slave_Block.png" alt="OBI Slave Block Diagram">
  <figcaption>Figure 4: OBI Slave Block Diagram</figcaption>
</figure>

The goal is to create a slave that can efficiently service pipelined read and write requests from a master, as illustrated in the following waveforms:

<figure>
  <img src="/assets/images/18- OBI_Write_Pipeline.svg" alt="OBI Slave Write Pipeline">
  <figcaption>Figure 5: OBI Slave Write Pipeline</figcaption>
</figure>

<figure>
  <img src="/assets/images/18- OBI_Read_Pipeline.svg" alt="OBI Slave Read Pipeline">
  <figcaption>Figure 6: OBI Slave Read Pipeline</figcaption>
</figure>

### Design Specification

We will design a slave with the following features:

- **Immediate Single-Cycle Responses**: It acknowledges requests in the same cycle they are made.
- **Full Pipelining**: It can accept a new request every clock cycle.
- **Always-Grant Policy**: The slave is always ready and never stalls the master (`gnt` is always high).
- **Byte-Enable Support**: It allows partial word writes.
- **Error Handling**: It returns an error for out-of-bounds memory accesses.

### Implementation in SystemVerilog

Let's break down the SystemVerilog implementation of our slave controller.

#### 1. Module Interface and Memory

First, we define the module with its parameters and ports, which match the OBI signal table. Inside, we declare a simple byte-addressable memory array.

```verilog
module obi_slave_custom #(
    parameter int unsigned ADDR_WIDTH = 32,
    parameter int unsigned DATA_WIDTH = 32,
    parameter int unsigned MEM_SIZE = 1024 // 1KB memory
) (
    // OBI interface ports...
);
    ...
    assign addr_valid = (obi_addr_i <= MEM_SIZE - 4);
```

#### 2. The "Always-Grant" Policy

To maximize throughput, our slave will always be ready to accept requests. This is achieved with a single line of code.
...
We use combinational logic to read from our memory. If the address is valid, we construct a 32-bit word from four bytes. Otherwise, we return zero.
...
Writes are handled in a sequential block. On a valid write request (`obi_req_i && obi_we_i`), we iterate through the byte enables (`obi_be_i`). If a byte enable bit is set, the corresponding byte in memory is updated.
...
Finally, we generate the response. The slave must assert `obi_rvalid_o` one cycle after a request is granted. For reads, we output the `read_data`; for writes, `rdata` is zero. The `err` signal is asserted if the address was invalid.
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (obi_req_i) begin
            obi_rvalid_o <= 1'b1;
            obi_err_o    <= !addr_valid;
            if (obi_we_i) begin
                obi_rdata_o <= '0;  // Write responses have no data
            end else begin
                obi_rdata_o <= read_data;  // Read responses return data
            end
        end else begin
            obi_rvalid_o <= 1'b0;
        end
...
<figure>
  <img src="/assets/images/18- OBI_Slave_Waveform.png" alt="OBI Slave Waveform">
  <figcaption>Figure 7: OBI Slave Simulation Waveform</figcaption>
</figure>

And that's it! We have a fully functional, pipelined OBI slave.

---

## Part 4: Designing an OBI Master
